...
function loadContent(url){
	$.ajax({type:"POST",url:url,data:"ajax=true",timeout:5000,error:function(){alert("头都摇晕了o(╯□╰)o 请自行刷新一下页面 >.<")},beforeSend:function(){$("#loader").show();$("object,embed").hide()},success:function(result){bodyclass=(/<body class=\"(.+)\">/).exec(result)[1];$("body").attr("class",bodyclass);new_title=(/<title>(.+)<\/title>/).exec(result)[1];$(document).attr("title",new_title);new_wpfooter=(/id=\"wpfooter\">([\s\S]*)<\/wpfooter>/).exec(result)[1];$("#wpfooter").html(new_wpfooter);new_mobnav=(/id=\"mobnav\">([\s\S]*)<\/mobnav>/).exec(result)[1];$("#mobnav").html(new_mobnav);$("nav").html($(result).find("nav").html());$("#content").html($(result).find("#content").html());$("#loader").hide();reloadJs();if(target=location.hash){target=target.substr(target.indexOf("#")+1);$("body").animate({scrollTop:$("a[name='"+target+"'],#"+target+"").offset().top})}else{$("body").animate({scrollTop:0})}initialUrl="http://"+location.host+location.pathname;window.onpopstate=function(){var newUrl="http://"+location.host+location.pathname;if(initialUrl==newUrl){return}loadContent(location.href)}}});
}
$(document).ready(function(){
	$("body").on("click","a",function(e){var link_uri=$(this).attr("href"),link_url=this.href;if(link_url.indexOf("<?php echo $_SERVER['SERVER_NAME'] ?>")>=0&&link_url.indexOf("/wp-")<0&&link_uri.indexOf("#respond")<0&&link_uri.charAt(0)!="#"){e.preventDefault();window.history.pushState(null,null,link_url);loadContent(link_url)}});
});
<?php 
    }
